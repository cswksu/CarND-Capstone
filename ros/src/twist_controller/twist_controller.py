...
class Controller(object):
    def __init__(self, vehParams):
        ...
        self.min_brake_torque = 800 #N*m to prevent idle crawl
    ...
    def control(self, current_velocity, proposed_linear_velocity, proposed_angular_velocity, drive_by_wire_enabled):
        ...
        if proposed_linear_velocity and proposed_angular_velocity and current_velocity:
            dragThrot=0#-0.1*current_velocity/5.3
            ...
            if throt_temp>dragThrot:
                ...
                brake=(-throt_temp+10*dragThrot)*9.81*self.torque_inertia
                throt=0
            #if (abs(current_velocity)<=(5.0*ONE_MPH)) and (abs(proposed_linear_velocity)<=(2.0*ONE_MPH)):
            if proposed_linear_velocity<2.0:
                brake=max(self.min_brake_torque,brake)
                throt=0
